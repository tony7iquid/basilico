...
add_action('wp_ajax_basilico_get_pagination_html', 'basilico_get_pagination_html');
add_action('wp_ajax_nopriv_basilico_get_pagination_html', 'basilico_get_pagination_html');
function basilico_get_pagination_html()
{
    try {
        if (!isset($_POST['query_vars'])) {
            throw new Exception(__('Something went wrong while requesting. Please try again!', 'basilico'));
        }
        $query = new WP_Query($_POST['query_vars']);
        ob_start();
        basilico()->page->get_pagination($query,  true);
        $html = ob_get_clean();
        wp_send_json(
            array(
                'status' => true,
                'message' => esc_attr__('Load Successfully!', 'basilico'),
                'data' => array(
                    'html' => $html,
...
add_action('wp_ajax_basilico_get_filter_html', 'basilico_get_filter_html');
add_action('wp_ajax_nopriv_basilico_get_filter_html', 'basilico_get_filter_html');
function basilico_get_filter_html()
{
    try {
        ...
        $settings['filter_default_title'] = !empty($settings['filter_default_title']) ? $settings['filter_default_title'] : esc_html__('All', 'basilico');
        ob_start();
...
add_action('wp_ajax_basilico_load_more_product_grid', 'basilico_load_more_product_grid');
add_action('wp_ajax_nopriv_basilico_load_more_product_grid', 'basilico_load_more_product_grid');
function basilico_load_more_product_grid()
{
    try {
        if (!isset($_POST['settings'])) {
            throw new Exception(__('Something went wrong while requesting. Please try again!', 'basilico'));
        ...
        if ($posts->have_posts()) {
            ...
            wp_send_json(
                array(
                    'status' => true,
                    'message' => esc_html__('Load Post Grid Successfully!', 'basilico'),
                    'data' => array(
                        'html'  => $html,
                        'paged' => $settings['paged'],
                        'posts' => $posts,
            ...
            wp_send_json(
                array(
                    'status' => false,
                    'message' => esc_html__('Load Post Grid No More!', 'basilico')
                )
            );
        }
    } catch (Exception $e) {
...
add_action('wp_ajax_basilico_load_more_post_grid', 'basilico_load_more_post_grid');
add_action('wp_ajax_nopriv_basilico_load_more_post_grid', 'basilico_load_more_post_grid');
function basilico_load_more_post_grid()
{
    try {
        if (!isset($_POST['settings'])) {
            throw new Exception(__('Something went wrong while requesting. Please try again!', 'basilico'));
        ...
        basilico_get_post_grid($posts, $settings);
        $html = ob_get_clean();
        wp_send_json(
            array(
                'status' => true,
...
if (!function_exists('basilico_get_post_grid')) {
    function basilico_get_post_grid($posts = [], $settings = [])
    {
            ...
            foreach ($posts as $key => $post) {
                if (!empty($grid_custom_columns[$key])) {
                    ...
                    if (!empty($grid_custom_columns[$key]['item_c_animation'])) {
                        ...
                        $anm_cls = ' pxl-animate pxl-invisible animated-' . $grid_custom_columns[$key]['item_c_animation_duration'];
        ...
        switch ($layout) {
            case 'post-list-1':
            basilico_get_post_list_layout1($posts, $settings, $args_m);
            break;
            case 'post-list-2':
            basilico_get_post_list_layout2($posts, $settings, $args_m);
            break;
            case 'post-list-3':
...
function basilico_get_post_list_layout1($posts = [], $settings = [], $args_m = [])
{
    extract($settings);
    foreach ($posts as $key => $post) :
        ...
        $button_text = !empty($button_text) ? $button_text : esc_html__('Continue reading', 'basilico');
        $increase = $key + 1;
        $data_settings = '';
        $animate_cls = '';
...
function basilico_get_post_list_layout2($posts = [], $settings = [], $args_m = [])
{
    extract($settings);
    foreach ($posts as $key => $post) :
...
function basilico_get_post_list_layout3($posts = [], $settings = [], $args_m = [])
{
    extract($settings);
    foreach ($posts as $key => $post) :
        $str_item_class = !empty($args_m[$key]['item_class']) ? $args_m[$key]['item_class'] : $item_class;
...
function basilico_get_post_list_layout4($posts = [], $settings = [], $args_m = [])
{
    extract($settings);
    foreach ($posts as $key => $post) :
        $str_item_class = !empty($args_m[$key]['item_class']) ? $args_m[$key]['item_class'] : $item_class;
...
function basilico_get_post_grid_layout1($posts = [], $settings = [], $args_m = [])
{
    extract($settings);
    foreach ($posts as $key => $post) :
        $str_item_class = !empty($args_m[$key]['item_class']) ? $args_m[$key]['item_class'] : $item_class;
...
function basilico_get_post_grid_layout2($posts = [], $settings = [], $args_m = [])
{
    extract($settings);
    foreach ($posts as $key => $post) :
        $str_item_class = !empty($args_m[$key]['item_class']) ? $args_m[$key]['item_class'] : $item_class;
                    ...
                    <div class="item-content col-lg-6">
                        ...
                        if ($show_tag == 'true') {
                            ...
                            <div class="post-metas">
                                <div class="meta-inner d-flex align-items-center">
                                    <span class="post-tag">
                                        <?php the_terms($post->ID, 'pxl-portfolio-tag', '', ',&nbsp', ''); ?>
                                    </span>
                                </div>
                            </div>
                            <?php
