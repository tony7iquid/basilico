...
if (!class_exists('Basilico_Woo')) {
    class Basilico_Woo
    {
    	...
    	public function get_single_product_images($product,$args){
			...
			$product_feature_video = basilico()->get_page_opt('product_feature_video', 'no');
			$video_light_gallery   = isset($_GET['video-lightbox']) ? sanitize_text_field( wp_unslash($_GET['video-lightbox'])) : basilico()->get_page_opt('video_light_gallery', 'off');  
			$video_output = '';
			if( $product_feature_video != 'no'){
				global $wp_embed;
				$video_url             = basilico()->get_page_opt('video_url', '');
				$video_file            = basilico()->get_page_opt('video_file', []);  
				$video_html            = basilico()->get_page_opt('video_html', '');  
    		...
    		<?php if( $product_layout == 'layout-df'): 
    			...
    			if ( !empty( $gal_ids ) ){
    				$wrap_cls[] = 'pxl-product-swiper-slider pxl-swiper-slider has-thumbs-gallery thumbs-vertical-slider';
    			}

    			$zoom_img_hover = basilico()->get_theme_opt('zoom_img_hover', 'on');
    			$wrap_cls[] = !empty($video_output) ? 'video-feature' : '';
    			?>
	    		
    			<div class="<?php echo esc_attr( trim(implode(' ', $wrap_cls) )) ?>">
	    			<div class="product-main-img pxl-light-gallery">
	    				<div class="product-main-img-inner relative">
							...
							if ( !empty( $gal_ids ) ){
						            ...
						            <div class="pxl-swiper-container" data-settings="<?php echo esc_attr($data_settings) ?>" data-rtl="<?php echo esc_attr($dir) ?>">
						                <div class="pxl-swiper-wrapper swiper-wrapper">
						                	...
						                	foreach ($gallery_ids as $ga_id) : 
												...
												<div class="pxl-swiper-slide swiper-slide <?php echo esc_attr($item_cls) ?>" data-src="<?php echo esc_url($data_src) ?>" data-image-id="<?php echo esc_attr($ga_id) ?>" data-sub-html="<?php echo esc_attr( $sub_html ) ?>">
													...
													<?php else: ?>
														...
														Basilico_Image::pxl_image_by_size( array(
													 	...
													 	<?php if(!empty( $video_output) && $k == 1 && $video_light_gallery == 'on'):?>
													 		<?php echo '<span class="pxl-item-video zoom" data-src="'.esc_url($video_url).'"><span class="pxli-play"></span></span>'; ?>
													 	<?php endif; ?> 
													 	<?php if($zoom_img_hover == 'on'): ?>
													 	</div>
													 	<?php endif; ?> 
						        ...
						        <div class="pxl-swiper-arrows nav-in-vertical arrow-on-hover">
					                <div class="pxl-swiper-arrow pxl-swiper-arrow-prev"><span class="pxl-icon lnil lnil-chevron-left"></span></div>
            						<div class="pxl-swiper-arrow pxl-swiper-arrow-next"><span class="pxl-icon lnil lnil-chevron-right"></span></div>
					            </div>
								<?php 
							}else{
								echo esc_attr($this->get_product_image( $product, $image_size )); 
							}
							?>
					...
					<?php if ( ! empty( $gal_ids ) ) : 
						...
						<div class="product-gallery-img "> 
							<div class="product-gallery-carousel pxl-swiper-slider">
								<div class="pxl-swiper-slider-inner pxl-carousel-inner overflow-hidden">
									<div class="swiper-container-thumbs pxl-swiper-thumbs" data-settings="<?php echo esc_attr($data_thumb_settings) ?>" data-rtl="<?php echo esc_attr($dir) ?>">
				                		<div class="pxl-swiper-wrapper swiper-wrapper">
											<?php foreach ($gallery_ids as $ga_id) : ?>
												<div class="pxl-swiper-slide swiper-slide" data-image-id="<?php echo esc_attr($ga_id) ?>">
													...
													Basilico_Image::pxl_image_by_size( array(
												 	...
												 	<?php if(!empty( $video_output) && $k == 1):?>
												 		<?php echo '<span class="pxl-item-video" data-src="'.esc_url($video_url).'"><span class="pxli-play"></span></span>'; ?>
												 	<?php endif; ?> 
												 	</div>
											 	</div>
											<?php endforeach; ?>
			...
			<?php if( $product_layout == 'layout-sticky'):
			 	$image_size = ! empty( $image_size ) ? $image_size : $image_width.'x820';
			 	if ( !empty( $gal_ids ) ){
    				$wrap_cls[] = 'pxl-product-sticky has-thumbs-gallery thumbs-vertical-left';
    				/*if ( $product_image_id = $product->get_image_id() ) { 
    					$gallery_ids = array_merge([$product_image_id], $gallery_ids) ;
    				}*/
    			}
    			$zoom_img_hover = basilico()->get_theme_opt('zoom_img_hover', 'on');
    			$gal_display_type = isset($_GET['img-display-type']) ? sanitize_text_field( wp_unslash($_GET['img-display-type'])) : basilico()->get_opt('product_grid_gal_display_type','all');
					...
					<?php endif; ?>
			 		<div class="product-main-img pxl-light-gallery">
	    				<div class="product-main-img-inner relative">
							...
							if ( !empty( $gal_ids ) ){
								?>	
				                <div class="pxl-img-sticky-wrapper d-flex flex-column">
				                	<?php 
				                	$k =  0;
				                	foreach ($gallery_ids as $ga_id) : 
				                		$k++;
			...
			<?php if( $product_layout == 'layout-with-sidebar'):
				$image_size = ! empty( $image_size ) ? $image_size : $image_width.'x820';
    			if ( !empty( $gal_ids ) ){
    				$wrap_cls[] = 'pxl-product-swiper-slider pxl-swiper-slider has-thumbs-gallery thumbs-horizontal-slider';
    				/*if ( $product_image_id = $product->get_image_id() ) { 
    					$gallery_ids = array_merge([$product_image_id], $gallery_ids) ;
    				}*/
    			}
    			...
    			<div class="<?php echo esc_attr( trim(implode(' ', $wrap_cls) )) ?>">
						            ...
						            <div class="pxl-swiper-container" data-settings="<?php echo esc_attr($data_settings) ?>" data-rtl="<?php echo esc_attr($dir) ?>">
						                <div class="pxl-swiper-wrapper swiper-wrapper">
						                	...
						                	foreach ($gallery_ids as $ga_id) : 
												...
												<div class="pxl-swiper-slide swiper-slide zoom <?php echo esc_attr($item_cls) ?>" data-src="<?php echo esc_url($data_src) ?>" data-image-id="<?php echo esc_attr($ga_id) ?>" data-sub-html="<?php echo esc_attr( $sub_html ) ?>">
													<?php if(!empty( $video_output) && $video_light_gallery == 'off' && $k == 1 ): ?>
														<?php echo ''.$video_output; ?>
													<?php else: ?>
														<?php if($zoom_img_hover == 'on'): ?>
					...
					<?php if ( !empty( $gal_ids ) ): 
						...
						?>
						<div class="product-gallery-img"> 
							<div class="product-gallery-carousel pxl-swiper-slider relative">
								<div class="pxl-swiper-slider-inner pxl-carousel-inner overflow-hidden">
									<div class="swiper-container-thumbs pxl-swiper-thumbs" data-settings="<?php echo esc_attr($data_thumb_settings) ?>" data-rtl="<?php echo esc_attr($dir) ?>">
				                		<div class="pxl-swiper-wrapper swiper-wrapper">
											<?php foreach ($gallery_ids as $ga_id) : ?>
			...
			<?php if( $product_layout == 'layout-full-width'):
			 	$image_size = ! empty( $image_size ) ? $image_size : $image_width.'x820';
			 	if ( !empty( $gal_ids ) ){
    				$wrap_cls[] = 'pxl-product-list-gallery';
    			}
    			$zoom_img_hover = basilico()->get_theme_opt('zoom_img_hover', 'on');
    			$gal_display_type = isset($_GET['img-display-type']) ? sanitize_text_field( wp_unslash($_GET['img-display-type'])) : basilico()->get_opt('product_grid_gal_display_type','all');
    			$display_type_cls = $gal_display_type == 'by-attribute' ? 'display-by-attribute' : 'display-all';
			 	...
			 	<div class="<?php echo esc_attr( trim(implode(' ', $wrap_cls) )) ?>">
			 		<div class="product-main-img pxl-light-gallery">
	    				<div class="product-main-img-inner relative">
							...
							if ( !empty( $gal_ids ) ){
								?>	
				                <div class="pxl-img-list-wrapper d-flex flex-column">
				                	<?php 
				                	$k =  0;
				                	foreach ($gallery_ids as $ga_id) : 
				                		$k++;
			...
			<?php if( $product_layout == 'layout-grid-gallery'):
			 	$image_size = ! empty( $image_size ) ? $image_size : $image_width.'x820';
			 	if ( !empty( $gal_ids ) ){
    				$wrap_cls[] = 'pxl-product-grid-gallery';
    			}
    			$zoom_img_hover = basilico()->get_theme_opt('zoom_img_hover', 'on');
    			$gal_display_type = isset($_GET['img-display-type']) ? sanitize_text_field( wp_unslash($_GET['img-display-type'])) : basilico()->get_opt('product_grid_gal_display_type','all');
    			$display_type_cls = $gal_display_type == 'by-attribute' ? 'display-by-attribute' : 'display-all';
	    				...
	    				<div class="product-main-img-inner relative">
							...
							if ( !empty( $gal_ids ) ){
								$row_cols_class = basilico_get_shop_loop_row_column_class([
								    'col_xs'  => basilico()->get_opt('products_grid_gal_col_xs', '2'),
								    'col_sm'  => basilico()->get_opt('products_grid_gal_col_sm', '2'),  
								    'col_md'  => basilico()->get_opt('products_grid_gal_col_md', '2'), 
								    'col_lg'  => basilico()->get_opt('products_grid_gal_col_lg', '2'),
								    'col_xl'  => basilico()->get_opt('products_grid_gal_col_xl', '2'),  
								    'col_xxl' => basilico()->get_opt('products_grid_gal_col_xxl', '2')
								]); 
								
								$g_x_cls = basilico_get_grid_gutter_x_class('products_grid_gal_gutter_x', true);
								$g_y_cls = basilico_get_grid_gutter_y_class('products_grid_gal_gutter_y', true);
								$g_x_cls = !empty($g_x_cls) ? $g_x_cls : ['gx-15'];
								$g_y_cls = !empty($g_y_cls) ? $g_y_cls : ['gy-15'];
								?>	
				                <div class="pxl-img-grid-wrapper row relative <?php echo esc_attr(implode(' ', $g_x_cls)) ?> <?php echo esc_attr(implode(' ', $g_y_cls)) ?> <?php echo esc_attr(implode(' ', $row_cols_class)) ?>">
				                	<?php 
				                	$k =  0;
				                	foreach ($gallery_ids as $ga_id) : 
				                		$k++;
    	...
    	public function get_loop_product_thumbnail($product,$args){ 
    		...
    		$shop_archive_hover_image = isset($args['shop_archive_hover_image']) ? $args['shop_archive_hover_image'] : basilico()->get_theme_opt('shop_archive_hover_image', '0');
    		$thumb_class = '';
    		...
    		<div class="thumb-wrap <?php echo esc_attr($thumb_class) ?>">
				...
				<div class="product-main-img">
					<?php echo esc_attr($this->get_product_image( $product, $thumbnail_size )); ?>
				</div>
				<?php if ( $has_hover_thumbnail ) { ?>
					<div class="product-hover-img pxl-absoluted">
						...
						<?php if($shop_archive_hover_image == '2'): ?>
							<div class="product-loop-carousel pxl-swiper-slider pxl-theme-carousel">
								<div class="pxl-swiper-slider-inner pxl-carousel-inner overflow-hidden">
									<div class="pxl-swiper-container" data-settings="<?php echo esc_attr($data_settings) ?>" data-rtl="<?php echo esc_attr($dir) ?>">
	                            		<div class="pxl-swiper-wrapper swiper-wrapper">
											<?php foreach ($gallery_ids as $ga_id) : ?>
		...
		public function get_active_filter_remove_links( $filters ) {
			...
			foreach ( $filters as $filter_name => $value ) {
				...
				if ( ! empty( $taxonomy_name ) && taxonomy_exists( $taxonomy_name ) ) {
					...
					foreach ( $filter_terms as $key => $term_id ) {
						...
						$tooltip_text = isset( $taxonomy->labels->singular_name ) ? $taxonomy->labels->singular_name : __( 'Filter', 'basilico' );

						$clear_links[] = [
							'url'     => $clear_link,
							'text'    => $term->name,
							'tooltip' => sprintf( __( 'Remove This %s', 'basilico' ), $tooltip_text ),
					...
					foreach ( $filter_terms as $key => $term_slug ) {
						...
						$clear_links[] = [
							'url'     => $clear_link,
							'text'    => $term->name,
							'tooltip' => sprintf( __( 'Remove This %s', 'basilico' ), $attribute_info->name ),
							'class'   => 'remove-filter-link',
						];
					}
					...
					foreach ( $filter_values as $key => $filter_value ) {
						...
						$clear_links[] = [
							'url'     => $clear_link,
							'text'    => sprintf( _n( '%s star', '%s stars', $filter_value, 'basilico' ), $filter_value ),
							'tooltip' => sprintf( __( 'Remove This %s', 'basilico' ), __( 'Rating', 'basilico' ) ),
							'class'   => 'remove-filter-link',
						];
					}
					...
					$clear_links[] = [
						...
						'tooltip' => sprintf( __( 'Remove This %s', 'basilico' ), __( 'Highlight', 'basilico' ) ),
						'class'   => 'remove-filter-link',
					];
				}*/
			}
			...
			if ( isset( $filters['min_price'] ) && isset( $filters['max_price'] ) ) {
				...
				$clear_links[] = [
					...
					'tooltip' => sprintf( __( 'Remove This %s', 'basilico' ), __( 'Price', 'basilico' ) ),
					'class'   => 'remove-filter-link',
				];
			}
 
			$output = '<div class="pxl-active-filters-inner">';
			...
			if ( ! empty( $clear_links ) ) {
				$clear_all_links = [
					...
					'text'  => '<span class="lnil lnil-reload"></span>'.esc_html__( 'Reset All', 'basilico' ),
					'class' => 'reset-all',
					'tooltip' => esc_html__( 'Remove This', 'basilico' ),
				];
  		...
  		public function get_category_list_by_selected($args = []){
            ...
            foreach ($terms as $term): 
            	...
            	$count       = Pxl_Woo_Query::instance()->get_hierarchy_tax_counts( [$term->term_id], 'product_cat', 'or', false  );
            	if ( $count <= 0 ) {
					continue;
				}  
  		...
  		public function get_category_list_filter($wg_id = '', $settings = []){
			...
			?>
			<div id="<?php echo esc_attr($wg_id) ?>" class="widget pxl-widget pxl-widget-product-categories pxl-widget-wc-layered-nav">
			...
			$chosen_terms   = $this->get_chosen_terms( 'filter_product_cat' );
  			echo '<ul class="' . esc_attr( implode( ' ', $class ) ) . '">';
        		...
        		foreach ( $terms as $term ) {
					...
					$count       = Pxl_Woo_Query::instance()->get_hierarchy_tax_counts( $child_ids, 'product_cat', 'or', false  );
					...
					$item_class = [ 'pxl-layered-nav-term pxl-list-item' ];
		...
		public function get_sortby_list_filter($wg_id = '', $settings = []){
			extract($settings);
  			$orderby_options = array(
				'popularity' => esc_html__( 'Popularity', 'basilico' ),
				'rating'     => esc_html__( 'Average Rating', 'basilico' ),
				'sale'       => esc_html__( 'Sale', 'basilico' ),
				'date'       => esc_html__( 'New In', 'basilico' ),
				'price'      => esc_html__( 'Price Low to High', 'basilico' ),
				'price-desc' => esc_html__( 'Price High to Low', 'basilico' ),
            );
            $base_link = $this->get_shop_active_filters_url();
            ...
            ?>
            <div id="<?php echo esc_attr($wg_id) ?>" class="widget pxl-widget pxl-widget-product-orderby pxl-widget-wc-layered-nav">
                <div class="widget-content">
                	<?php if(!empty($wg_title)): ?>
                		<h4 class="widget-title"><span><?php echo esc_html($wg_title) ?></span></h4>
                	<?php endif; ?>
                    <div class="widget-content-inner <?php echo esc_attr($content_class) ?>">
                    	<ul class="sortby-filter">
				            ...
				            foreach ( $orderby_options as $id => $name ){
								...
								$item_class = [ 'pxl-layered-nav-term pxl-list-item orderby' ];
								$link_class = 'filter-link';
  						...
  						<form method="get" action="<?php echo esc_url( $form_action ); ?>">
			                <div class="price-filter-widget price_slider_wrapper">
			                    ...
			                    <div class="price_slider_amount" data-step="<?php echo esc_attr( $step ); ?>"> 
			                    	<label class="screen-reader-text" for="min_price"><?php esc_html_e( 'Min price', 'basilico' ); ?></label>
									<input type="text" id="min_price" name="min_price" value="<?php echo esc_attr( $current_min_price ); ?>" data-min="<?php echo esc_attr( $min_price ); ?>" placeholder="<?php echo esc_attr__( 'Min price', 'basilico' ); ?>" />
									<label class="screen-reader-text" for="max_price"><?php esc_html_e( 'Max price', 'basilico' ); ?></label>
									<input type="text" id="max_price" name="max_price" value="<?php echo esc_attr( $current_max_price ); ?>" data-max="<?php echo esc_attr( $max_price ); ?>" placeholder="<?php echo esc_attr__( 'Max price', 'basilico' ); ?>" />
				                    <button type="submit" class="pxl-btn btn-outline button"><?php echo esc_html__( 'Filter', 'basilico' ); ?></button>
  		...
  		public function get_filtered_price() {
			...
			//$tax_query  = Pxl_Woo_Query::instance()->get_main_tax_query();
			//$meta_query = Pxl_Woo_Query::instance()->get_main_meta_query();

			$tax_query       = new WP_Tax_Query( $tax_query );
			$meta_query      = new WP_Meta_Query( $meta_query );

			$search     = Pxl_Woo_Query::instance()->get_main_search_query_sql();
		...
		public function get_attribute_filter($wg_id = '', $settings = []){
			...
			$term_counts = Pxl_Woo_Query::instance()->get_filtered_term_product_counts(wp_list_pluck( $terms,'term_id' ), $taxonomy, $query_type, false );
			...
			?>
			<div id="<?php echo esc_attr($wg_id) ?>" class="widget pxl-widget widget_layered_nav woocommerce-widget-layered-nav pxl-widget-layered-nav">
                <div class="widget-content">
                	<?php if(!empty($wg_title)): ?>
                		<h4 class="widget-title"><span><?php echo esc_html($wg_title) ?></span></h4>
                	<?php endif; ?>
									...
									if ( 'swatches' === $list_style ) :
										switch ( $attr_info->type ) :
											case 'color':
												...
												$item_link_class .= ' pxl-ttip tt-top';
					...
					if( $enable_load_more == '1' && count($term_counts) > $items_loadmore_display){  
						echo '<span class="pxl-toogle-more" data-display="'.esc_attr($items_loadmore_display).'" data-less="'.esc_attr__( 'Less', 'basilico' ).'" data-more="'.esc_attr__( 'More', 'basilico' ).'" ><span class="text">' . esc_html__( 'More', 'basilico' ) . '</span><span class="pxl-icon lnil lnil-chevron-down"></span></span>';
					}
                    ?>
                </div>
            </div>
		...
		public function get_recent_viewed_products(){
			...
			$viewed_pids = [];
			$posts_per_page     = (int)basilico()->get_theme_opt('product_recent_viewed_per_page', 8);
		...
		public function get_cross_sell_product(){
			...
			$cross_sells_ids = basilico()->get_page_opt('product_cross_sell_ids', []);
